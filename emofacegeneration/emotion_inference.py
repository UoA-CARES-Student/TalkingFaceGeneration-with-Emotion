...
def main():
    ...
    parser.add_argument('--gen_dir', help='path to directory results will be outputted to. by default this path is: ./faces/generated_img ', default= './faces/generated_img', type = str)
    ...
    if os.path.exists(args.gen_dir):
        ...
        print("I found: " + str(glob.glob(mask_path + args.face_name + "*")))
        ...
        if os.path.exists(latent_img_path[0]):
            generate_emotion(latent_img_path[0], mask_img_path[0],aligned_img_path[0], args.coeff, args.gen_dir)
        else:
