...
class PerceptualModel:
    def __init__(self, percep_dict, mask_dict, batch_size=1, perc_model=None, sess=None):
        self.sess = tf.get_default_session() if sess is None else sess
        K.set_session(self.sess)
        self.epsilon = 0.00000001
        self.lr =percep_dict["lr"]
        self.decay_rate = percep_dict["decay_rate"]
        self.decay_steps = percep_dict["decay_steps"]
        self.img_size = percep_dict["image_size"]
        self.layer = percep_dict["use_vgg_layer"]
        self.vgg_loss = percep_dict["use_vgg_loss"]
        self.face_mask = mask_dict["face_mask"]
        self.use_grabcut = mask_dict["use_grabcut"]
        self.scale_mask = mask_dict["scale_mask"]
        self.mask_dir = mask_dict["mask_dir"]
        if (self.layer <= 0 or self.vgg_loss <= self.epsilon):
            self.vgg_loss = None
        self.pixel_loss = percep_dict["use_pixel_loss"]
        if (self.pixel_loss <= self.epsilon):
            self.pixel_loss = None
        self.mssim_loss = percep_dict["use_mssim_loss"]
        if (self.mssim_loss <= self.epsilon):
            self.mssim_loss = None
        self.lpips_loss = percep_dict["use_lpips_loss"]
        if (self.lpips_loss <= self.epsilon):
            self.lpips_loss = None
        self.l1_penalty = percep_dict["use_l1_penalty"]
        if (self.l1_penalty <= self.epsilon):
            self.l1_penalty = None
        self.adaptive_loss = percep_dict["use_adaptive_loss"]
        self.sharpen_input = percep_dict["sharpen_input"]
        ...
        self.loss = None
        self.discriminator_loss = percep_dict["use_discriminator_loss"]
