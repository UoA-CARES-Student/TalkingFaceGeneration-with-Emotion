...
tflib.init_tf()
with open("/workspace/alan/EmoFaceGeneration/emofacegeneration/stylegan-encoder/weights/karras2019stylegan-ffhq-1024x1024.pkl", "rb") as f:
...
def move_and_show(latent_vector, mask_path, orig_img_path, emo_coeffs, emo_cond):

    # direction = np.load('ffhq_dataset/latent_directions/smile.npy')    
    if emo_cond == "happy":
        # direction_dir = HAPPY_DIR
        # direction = np.load('/workspace/alan/image_expr_mod/stylegan-encoder/latent_directions/emotion_easy.npy')
        np.load('ffhq_dataset/latent_directions/smile.npy')    
    if emo_cond == "sad":
        print("I loaded sad")
        # direction_dir = SAD_DIR
        direction = np.load('/workspace/alan/image_expr_mod/stylegan-encoder/latent_directions/emotion_sad.npy')
        # direction = np.load(direction_dir)


    # fig,ax = plt.subplots(1, len(coeffs), figsize=(15, 10), dpi=80)
    for i, coeff in enumerate(emo_coeffs):
        print(f"coeff is : {coeff}, coeff type is {type(coeff)}")
        ...
        cv2.imwrite("/workspace/alan/Eval/results/" + "coeff_" + str(coeff) + "_" + mask_path.split("/")[-1], img)
...
def generate_emotion(latent_path, mask_path, orig_img_path, emo_coeff, emo_cond):
    latent_vector = np.load(latent_path)
    move_and_show(latent_vector, mask_path, orig_img_path, emo_coeff, emo_cond)
